...
protected:
    void initializeInputs() override
    {
        ...
        top->funct3 = 0;
        top->funct7 = 0;
        // Note: 'eq' is not an input to control_unit, removed
    }
};
...
TEST_F(ControlUnit_tb, R_Type_MainDecoder)
{
    ...
    EXPECT_EQ(top->branch, 0);  // Changed from pc_src
...
// BEQ instruction: opcode = 1100011, funct3 = 000
TEST_F(ControlUnit_tb, BEQ_Decoding)
{
    ...
    EXPECT_EQ(top->branch, 0b01);  // branch[0] = 1 for BEQ
    EXPECT_EQ(top->alu_src, 0);
}

// BNE instruction: opcode = 1100011, funct3 = 001
TEST_F(ControlUnit_tb, BNE_Decoding)
{
    top->opcode = 0b1100011;
    top->funct3 = 0b001;  // BNE
    top->eval();

    // Check decoder outputs
    EXPECT_EQ(top->reg_write, 0);
    EXPECT_EQ(top->imm_src, 2);
    EXPECT_EQ(top->branch, 0b10);  // branch[1] = 1 for BNE
    EXPECT_EQ(top->alu_src, 0);
}
